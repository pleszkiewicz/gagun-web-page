window.gagunLocales = window.gagunLocales || {};

window.gagunLocales.en = {
  app: {
    title: "Gagun Web Page",
    eyebrow: "Gagun Tools",
  },
  navigation: {
    title: "Pages",
    open: "Open menu",
    close: "Close menu",
    sections: {
      ai: "AI Tools",
      mathematics: "Mathematics",
      fractals: "Fractals",
      chemistry: "Chemistry",
      games: "Games",
    },
    unavailableTitle: "Page not available",
    unavailableMessage: "This page is not registered yet.",
    desktopBadge: "Desktop",
    desktopOnlyTitle: "Desktop screen required",
    desktopOnlyMessage:
      "{page} needs a larger screen and keyboard controls. Open this page on a laptop or desktop computer to use it.",
    desktopOnlyMenuHint: "Requires a larger screen and keyboard controls.",
  },
  pages: {
    plotter2d: {
      navLabel: "2D Plotter",
      title: "2D Plotter",
      plotAreaLabel: "Plot area",
      toolbarLabel: "View tools",
      zoomOut: "Zoom out",
      zoomIn: "Zoom in",
      resetView: "Center",
      canvasLabel: "Function graph",
      eyebrow: "2D Plotter",
      heading: "Function Graphs",
      newFunction: "New function",
      addFunction: "Add",
      functionsLabel: "Added functions",
      functionsHeading: "Functions",
      syntaxLabel: "Syntax",
      syntaxHeading: "Syntax",
      toggleDerivative: "Toggle derivative of {name}(x)",
      showDerivativeAction: "Show",
      hideDerivativeAction: "Hide",
      derivativeUnavailableAction: "N/A",
      save: "Save",
      delete: "Delete",
      derivativeUnavailable: "Derivative unavailable.",
      derivativeUnavailableWithReason: "Derivative unavailable: {reason}",
      unsupportedOperator: "Operator \"{operator}\" is not supported.",
      unsupportedFunction: "Function \"{name}\" is not supported.",
      unknownSymbol: "Unknown symbol \"{name}\".",
      unsupportedElement: "Element \"{type}\" is not supported.",
      mathNotLoaded: "The math.js library has not loaded.",
      enterExpression: "Enter a function expression.",
      inequalityNeedsYAxis: "Use y or f(x) on one side of the inequality.",
    },
    plotter3d: {
      navLabel: "3D Plotter",
      title: "3D Plotter",
      plotAreaLabel: "3D plot area",
      toolbarLabel: "3D view tools",
      zoomOut: "Zoom out",
      zoomIn: "Zoom in",
      resetView: "Center",
      canvasLabel: "3D function graph",
      eyebrow: "3D Plotter",
      heading: "Surface Graphs",
      newFunction: "New function",
      addFunction: "Add",
      functionsLabel: "Added surfaces",
      functionsHeading: "Surfaces",
      syntaxLabel: "Syntax",
      syntaxHeading: "Syntax",
      delete: "Delete",
      hideSurface: "Hide {name}(x, y)",
      showSurface: "Show {name}(x, y)",
      unsupportedOperator: "Operator \"{operator}\" is not supported.",
      unsupportedFunction: "Function \"{name}\" is not supported.",
      unknownSymbol: "Unknown symbol \"{name}\".",
      unsupportedElement: "Element \"{type}\" is not supported.",
      mathNotLoaded: "The math.js library has not loaded.",
      threeNotLoaded: "The Three.js library has not loaded.",
      webglUnavailable: "WebGL is not available in this browser.",
      enterExpression: "Enter a function expression.",
      inequalitiesUnsupported: "Inequalities are not supported in the 3D plotter.",
      equalityNeedsSurface: "Use z or f(x, y) on one side of the equation.",
    },
    fourierEpicycles: {
      navLabel: "Fourier Epicycles",
      title: "Fourier Epicycles",
      pageLabel: "Fourier epicycles shape animation",
      canvasLabel: "Animated Fourier epicycles drawing a shape",
      panelLabel: "Fourier epicycles controls",
      eyebrow: "Fourier Series",
      heading: "Fourier Epicycles",
      readout: "{shape}",
      shapeLabel: "Shape",
      randomShape: "Random",
      uploadImage: "Upload image",
      customShape: "Custom image",
      epicyclesLabel: "Epicycles",
      speedLabel: "Speed",
      pause: "Pause",
      resume: "Resume",
      processingImage: "Processing image",
      imageLoaded: "Image contour loaded",
      uploadFailed: "Image contour unavailable",
      shapes: {
        dino: "Dino",
        octopus: "Octopus",
        optimusPrime: "Optimus Prime",
        pi: "PI",
      },
    },
    fractals: {
      navLabel: "Julia Sets",
      title: "Julia Sets",
      pageLabel: "Animated Julia sets",
      canvasLabel: "Animated black and white Julia set",
      panelLabel: "Fractal controls",
      eyebrow: "Fractals",
      heading: "Julia Sets",
      loopLabel: "Infinite loop",
      resolution: "Resolution",
      pause: "Pause",
      resume: "Resume",
    },
    reactionDiffusion: {
      navLabel: "Reaction-Diffusion",
      title: "Reaction-Diffusion",
      pageLabel: "Animated black and white reaction-diffusion simulation",
      canvasLabel: "Animated black and white reaction-diffusion pattern",
      panelLabel: "Reaction-diffusion controls",
      eyebrow: "Simulation",
      heading: "Reaction-Diffusion",
      parametersLabel: "Reaction-diffusion parameters",
      feed: "Feed",
      feedTooltip:
        "Adds fresh material to the simulation. Higher values make the pattern grow faster and often split into smaller cells.",
      kill: "Kill",
      killTooltip:
        "Removes the reacting material. Higher values thin the white bands and can make the pattern break apart.",
      diffusionB: "Diffusion B",
      diffusionBTooltip:
        "Controls how far the second chemical spreads. Higher values create softer, wider bands.",
      speed: "Speed",
      speedTooltip: "Runs more simulation steps per frame. Higher values animate faster.",
      resolution: "Resolution",
      resolutionTooltip:
        "Changes simulation detail. Higher values look sharper but need more processing.",
      threshold: "Threshold",
      thresholdTooltip:
        "Sets the black-white cutoff. Move it to change band thickness and contrast.",
      tooltipButton: "Show {parameter} hint",
      reseed: "Reseed",
      pause: "Pause",
      resume: "Resume",
    },
    barnsleyFern: {
      navLabel: "Barnsley Fern",
      title: "Barnsley Fern",
      pageLabel: "Point-rendered Barnsley fern",
      canvasLabel: "Green Barnsley fern rendered with points",
      panelLabel: "Barnsley fern parameters",
      eyebrow: "Fractals",
      heading: "Barnsley Fern",
      points: "{count} plotted points",
      probabilityLabel: "Probabilities",
      stemProbability: "Stem",
      mainFrondProbability: "Main frond",
      leftLeafletsProbability: "Left leaflets",
      rightLeafletsProbability: "Right leaflets",
    },
    sierpinski: {
      navLabel: "Sierpinski Triangle | Carpet",
      title: "Sierpinski Triangle | Carpet",
      pageLabel: "Point-rendered Sierpinski fractal",
      canvasLabel: "Sierpinski fractal rendered with points",
      panelLabel: "Sierpinski controls",
      eyebrow: "Fractals",
      triangleHeading: "Sierpinski Triangle",
      squareHeading: "Sierpinski Carpet",
      points: "{count} plotted points",
      modeLabel: "Fractal type",
      triangleMode: "Triangle",
      squareMode: "Square",
    },
    periodicTable: {
      navLabel: "Periodic Table",
      title: "Periodic Table",
      pageLabel: "Periodic table of chemical elements",
      tableLabel: "Periodic table grid",
      detailsLabel: "Element details",
      legendLabel: "Legend",
      eyebrow: "Chemistry",
      heading: "Periodic Table of Elements",
      description:
        "An interactive periodic table with atomic number, symbol, atomic mass, group, period, and chemical category.",
      atomicNumber: "Atomic number",
      symbol: "Symbol",
      atomicMass: "Atomic mass",
      group: "Group",
      period: "Period",
      separateSeries: "Separate series",
      lanthanideRange: "57-71 Lanthanides",
      actinideRange: "89-103 Actinides",
      categories: {
        "alkali-metal": "Alkali metals",
        "alkaline-earth-metal": "Alkaline earth metals",
        "transition-metal": "Transition metals",
        "post-transition-metal": "Post-transition metals",
        metalloid: "Metalloids",
        nonmetal: "Reactive nonmetals",
        halogen: "Halogens",
        "noble-gas": "Noble gases",
        lanthanide: "Lanthanides",
        actinide: "Actinides",
        unknown: "Unknown properties",
      },
      elements: {
        H: "Hydrogen",
        He: "Helium",
        Li: "Lithium",
        Be: "Beryllium",
        B: "Boron",
        C: "Carbon",
        N: "Nitrogen",
        O: "Oxygen",
        F: "Fluorine",
        Ne: "Neon",
        Na: "Sodium",
        Mg: "Magnesium",
        Al: "Aluminum",
        Si: "Silicon",
        P: "Phosphorus",
        S: "Sulfur",
        Cl: "Chlorine",
        Ar: "Argon",
        K: "Potassium",
        Ca: "Calcium",
        Sc: "Scandium",
        Ti: "Titanium",
        V: "Vanadium",
        Cr: "Chromium",
        Mn: "Manganese",
        Fe: "Iron",
        Co: "Cobalt",
        Ni: "Nickel",
        Cu: "Copper",
        Zn: "Zinc",
        Ga: "Gallium",
        Ge: "Germanium",
        As: "Arsenic",
        Se: "Selenium",
        Br: "Bromine",
        Kr: "Krypton",
        Rb: "Rubidium",
        Sr: "Strontium",
        Y: "Yttrium",
        Zr: "Zirconium",
        Nb: "Niobium",
        Mo: "Molybdenum",
        Tc: "Technetium",
        Ru: "Ruthenium",
        Rh: "Rhodium",
        Pd: "Palladium",
        Ag: "Silver",
        Cd: "Cadmium",
        In: "Indium",
        Sn: "Tin",
        Sb: "Antimony",
        Te: "Tellurium",
        I: "Iodine",
        Xe: "Xenon",
        Cs: "Cesium",
        Ba: "Barium",
        La: "Lanthanum",
        Ce: "Cerium",
        Pr: "Praseodymium",
        Nd: "Neodymium",
        Pm: "Promethium",
        Sm: "Samarium",
        Eu: "Europium",
        Gd: "Gadolinium",
        Tb: "Terbium",
        Dy: "Dysprosium",
        Ho: "Holmium",
        Er: "Erbium",
        Tm: "Thulium",
        Yb: "Ytterbium",
        Lu: "Lutetium",
        Hf: "Hafnium",
        Ta: "Tantalum",
        W: "Tungsten",
        Re: "Rhenium",
        Os: "Osmium",
        Ir: "Iridium",
        Pt: "Platinum",
        Au: "Gold",
        Hg: "Mercury",
        Tl: "Thallium",
        Pb: "Lead",
        Bi: "Bismuth",
        Po: "Polonium",
        At: "Astatine",
        Rn: "Radon",
        Fr: "Francium",
        Ra: "Radium",
        Ac: "Actinium",
        Th: "Thorium",
        Pa: "Protactinium",
        U: "Uranium",
        Np: "Neptunium",
        Pu: "Plutonium",
        Am: "Americium",
        Cm: "Curium",
        Bk: "Berkelium",
        Cf: "Californium",
        Es: "Einsteinium",
        Fm: "Fermium",
        Md: "Mendelevium",
        No: "Nobelium",
        Lr: "Lawrencium",
        Rf: "Rutherfordium",
        Db: "Dubnium",
        Sg: "Seaborgium",
        Bh: "Bohrium",
        Hs: "Hassium",
        Mt: "Meitnerium",
        Ds: "Darmstadtium",
        Rg: "Roentgenium",
        Cn: "Copernicium",
        Nh: "Nihonium",
        Fl: "Flerovium",
        Mc: "Moscovium",
        Lv: "Livermorium",
        Ts: "Tennessine",
        Og: "Oganesson",
      },
    },
    lines: {
      navLabel: "Lines",
      title: "Lines",
      pageLabel: "Lines game",
      stageLabel: "Empty Lines game stage",
      overlayLabel: "Lines player controls",
      keyboardLabel: "Keyboard control groups",
      canvasLabel: "Lines playfield",
      eyebrow: "Lines",
      heading: "Lines",
      players: "2-4 players",
      registeredPlayers: "{count}/4 ready",
      waiting: "Waiting",
      ready: "Ready",
      waitingForPlayers: "Waiting",
      waitingForSecondPlayer: "One more player",
      starting: "Starting",
      joinPrompt: "Press a player key to activate player",
      actionHint: "Press forward to shoot\nPress backward to block",
      countdownLabel: "Start in",
      winner: "{player} wins",
      noWinner: "No winner",
      winnerStatus: "Winner",
      out: "Out",
      gameOverTitle: "Game Over",
      gameOverDismiss: "Press any key",
      gameOverWins: "wins",
      gameOverSurvivalTime: "{time}s",
      playerOneLabel: "P1",
      playerOneKeys: "WASD",
      playerTwoLabel: "P2",
      playerTwoKeys: "Arrow keys",
      playerThreeLabel: "P3",
      playerThreeKeys: "IJKL",
      playerFourLabel: "P4",
      playerFourKeys: "TFGH",
      desktopOnlyMessage:
        "Lines needs a larger screen and keyboard controls. Open this page on a laptop or desktop computer to play.",
    },
    aiDialogs: {
      navLabel: "AI Dialogs",
      title: "AI Dialogs",
      pageLabel: "Moderated AI model discussion",
      setupLabel: "AI dialog setup",
      transcriptLabel: "AI dialog transcript",
      eyebrow: "AI Tools",
      heading: "AI Dialogs",
      description:
        "Run a concise, turn-based discussion between two selected models. API keys are used only for this browser session and are never exported.",
      topicLabel: "Discussion topic",
      topicPlaceholder: "Describe the problem, decision, or topic the models should discuss.",
      participantsEyebrow: "Participants",
      participantsHeading: "Choose exactly two models",
      selectionCount: "{count}/2 selected",
      providerHint: "Turn-based participant",
      modelLabel: "Model",
      apiKeyLabel: "API key",
      apiKeyPlaceholder: "{provider} API key",
      showKey: "Show API key",
      hideKey: "Hide API key",
      showKeyShort: "Show",
      hideKeyShort: "Hide",
      providerIdle: "Not selected",
      providerNeedsKey: "API key required",
      providerReady: "Ready",
      maxRounds: "Rounds",
      maxTokens: "Max tokens",
      temperature: "Temperature",
      securityNote:
        "Keys stay in memory only. They are sent to the same-origin proxy for each active model call and are never included in exports.",
      start: "Start discussion",
      stop: "Stop",
      newDiscussion: "New discussion",
      exportMd: "Export MD",
      exportJson: "Export JSON",
      statusReady: "Enter a topic and choose two keyed models.",
      statusRunning: "Discussion running.",
      statusSpeaker: "{provider} is preparing a concise reply.",
      statusStopped: "Discussion stopped.",
      statusComplete: "Discussion complete.",
      statusError: "Discussion stopped after an error.",
      validationTopic: "Enter a discussion topic.",
      validationParticipants: "Choose exactly two models.",
      validationKey: "Enter an API key for {provider}.",
      transcriptEyebrow: "Transcript",
      transcriptHeading: "Live discussion",
      roundReadout: "Round {round}/{total}",
      emptyTitle: "No discussion yet",
      emptyMessage: "The transcript will appear here once the two models start taking turns.",
      thinking: "{provider} is thinking",
      thinkingFallback: "Model is thinking",
      errorEndpoint: "The AI proxy is unavailable or returned an invalid response.",
      errorEmpty: "The model returned an empty response.",
      errorGeneric: "The model request failed.",
      errorTimeout: "The model did not respond before the turn timeout.",
    },
  },
};
